...
timeout 3s bash -c "until ping -c2 $target > /dev/null; do sleep 1; done"
if [[ $? -ne 0 ]]; then
...
while [ $count -le $total_pw ]; do
	...
	echo "Using password number $count from password list file..."
	...
	if [[ $? -eq 0 ]]; then
		...
		cat /home/ralongi/inf_ralongi/scripts/get_rhel_kernel_info.sh | sshpass -p $password ssh -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" root@$target 'bash -'
		rm -f $pw_file
		exit
	else
		let count++
...
cat /home/ralongi/inf_ralongi/scripts/get_rhel_kernel_info.sh | sshpass -p $password ssh -o "StrictHostKeyChecking= no" root@$target 'bash -'
