...
$dbg_flag
errata=${errata:-""}
skip_pssh_hosts_file_creation=${skip_pssh_hosts_file_creation:-"no"}
...
if [[ $skip_pssh_hosts_file_creation == "no" ]] && [[ ! -s $pssh_hosts_file ]]; then
	create_pssh_hosts_file
	wait
	check_pssh_hosts_file
else
	echo "Skipping pssh hosts file creation..."
fi
