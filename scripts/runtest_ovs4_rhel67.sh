# Script to set up environment for BOS lab and kick of test (/home/ralongi/Documents/scripts/runtest_ovs3.sh)

#!/bin/bash

sshpass -p redhat ssh root@qe-dell-ovs4.knqe.lab.eng.bos.redhat.com << "EOF"
	# the host still has the old hostname
	export HOSTNAME=qe-dell-ovs4.knqe.lab.eng.bos.redhat.com
	...
	export CLIENTS=qe-dell-ovs2.knqe.lab.eng.bos.redhat.com
	export SERVERS=qe-dell-ovs4.knqe.lab.eng.bos.redhat.com
	export NIC_DRIVER=cxgb4
	export NAY=no
	export PVT=yes
	export vm_os_variant=rhel6
	export image_name=rhel6.6.qcow2
	export img_vm=http://pnate-control-01.lab.bos.redhat.com/rhel6.6.qcow2
	export JOBID=778245
