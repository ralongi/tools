...
get_beaker_compose_id()
{
	...
	view_id=$(curl -sL https://beaker.engineering.redhat.com/distrotrees/?simplesearch=rhel-$rhel_minor_ver | grep '/distros/view' | egrep -v '\.n|\.d' | head -n1 | awk -F ">" '{print $1}' | awk -F "=" '{print $NF}' | tr -d '"')
	...
	latest_compose_id=$(curl -sL https://beaker.engineering.redhat.com/distrotrees/?simplesearch=rhel-$rhel_minor_ver | grep '/distros/view' | egrep -v '\.n|\.d' | head -n1 | awk -F ">" '{print $2}' | awk -F "<" '{print $1}')
...
echo "Provisioning $system_name ($arch) with distro $distro (Distro ID: $distro_id)"

if [[ $(bkr system-status $system_name | grep 'Current reservation' | awk '{print $NF}') != "None" ]] && [[ $(bkr system-status $system_name | grep 'Current loan' | awk '{print $NF}') != "None" ]]; then bkr system-release $system_name; fi
