get_if_driver_info()
{
    ...
    echo -e "Here is the output from "ip link list" for all interfaces on host $(hostname): "
    echo -e
    ip l l
    ...
    echo -e
    touch /tmp/iface_driver_list.txt
	...
	echo -e
	if [[ ! $(cat /tmp/iface_driver_list.txt | grep '!') ]]; then
		echo -e "Interfaces unavailable for test due to no detected link or port speed: NONE"
	else
