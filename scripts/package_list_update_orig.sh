...
echo "Enter OVS 2.13 RHEL-7 Version for FDP $fdp_release (112, etc.): "
read version
sedeasy "ovs_213_rhel7_ver" "$version" "$new_package_list_temp_file"
source "$new_package_list_temp_file"
http_code=$(curl --silent --head --write-out '%{http_code}' "$OVS213_RHEL7" | grep HTTP | awk '{print $2}')
if [[ "$http_code" -ne 200 ]]; then echo "$OVS213_RHEL7 is NOT a valid link.  Exiting..."; exit 1; fi
...
sed -i "s@^OVS213_$fdp_release"_"RHEL7=@OVS213_$fdp_release"_"RHEL7=${OVS213_RHEL7}@g" "$new_package_list_file"
echo ""
echo "Package location: $OVS213_RHEL7"
echo ""

echo "Enter OVS 2.13 RHEL-8 Version for FDP $fdp_release (139, etc.): "
read version
...
echo "Enter OVS 2.15 RHEL-8 Version for FDP $fdp_release (53, etc.): "
read version
...
echo "Enter OVS 2.16 RHEL-8 Version for FDP $fdp_release (32, etc.): "
read version
...
echo "Enter OVN 2.13 RHEL-8 Version for FDP $fdp_release (191, etc.): "
read version
...
echo "Enter OVN 2021 RHEL-8 Version for FDP $fdp_release (23, etc.): ": 
read version
