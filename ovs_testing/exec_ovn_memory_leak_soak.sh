...
IMG_GUEST=${IMG_GUEST:-"http://netqe-infra01.knqe.eng.rdu2.dc.redhat.com/share/vms/OVS/${image_name}"}
SRC_NETPERF=${SRC_NETPERF:-"http://netqe-infra01.knqe.eng.rdu2.dc.redhat.com/share/tools/netperf-20160222.tar.bz2"}
...
if [[ "$skip_rhel8_ovs213" != "yes" ]]; then
	...
	#lstest | runtest $compose  --repo "http://download.devel.redhat.com/nightly/latest-BUILDROOT-8-RHEL-8/compose/Buildroot/x86_64/os" --variant=BaseOS --arch=x86_64 --ks-meta "$ks_meta" --machine=$server,$client --systype=machine  --param=dbg_flag="set -x" --param=NAY=yes --param=NIC_DRIVER=$NIC_DRIVER --param=NIC_NUM=2 --param=RPM_OVS_SELINUX_EXTRA_POLICY=$RPM_OVS_SELINUX_EXTRA_POLICY --param=RPM_OVS=$RPM_OVS --param=RPM_OVS=$RPM_OVS --param=RPM_OVN_COMMON=$RPM_OVN_COMMON --param=loop_round=$loop_round --wb "$fdp_release, $ovs_rpm_name, $compose, openvswitch/ovn/soak_test, Driver: $NIC_DRIVER" --append-task="/kernel/networking/openvswitch/crash_check {dbg_flag=set -x}"
