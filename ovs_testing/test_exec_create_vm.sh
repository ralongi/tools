...
# create a VM and reserve system
...
if [[ "$arch_test" == "x86_64" ]]; then
	lstest $KERNEL_TESTS_HOME/kernel/networking/openvswitch/power_cycle_crash/set_config | runtest --task-fetch-url /distribution/check-install@ --fetch-url kernel@https://gitlab.cee.redhat.com/kernel-qe/kernel/-/archive/master/kernel-master.tar.bz2 $COMPOSE --ks-append="rootpw redhat" --product=$product --retention-tag=$retention_tag --arch=x86_64 --machine=$dut --topo=singleHost --systype=machine --param=ROLE="STANDALONE" --param=image_name=$VM_IMAGE --param=dbg_flag="$dbg_flag" --param=NIC_DRIVER=$NIC_DRIVER --param=RPM_OVS_SELINUX_EXTRA_POLICY=$RPM_OVS_SELINUX_EXTRA_POLICY --param=RPM_OVS=$RPM_OVS --wb "(Standalone: $dut), Create VM \`create VM Package Mode\`" --append-task=/distribution/reservesys --append-task=/kernel/networking/openvswitch/common/misc_tasks/recover_pxe_boot
	fi
elif [[ "$arch_test" == "aarch64" ]]; then
	if [[ $image_mode == "yes" ]]; then
		lstest $KERNEL_TESTS_HOME/kernel/networking/openvswitch/power_cycle_crash/set_config | runtest -B "64k" --task-fetch-url /distribution/check-install@ --fetch-url kernel@https://gitlab.cee.redhat.com/kernel-qe/kernel/-/archive/master/kernel-master.tar.bz2 $COMPOSE --watchdog-panic "ignore" --bootc=$COMPOSE --nrestraint --autopath --kernel-options "crashkernel=640M" --packages="virt-viewer,virt-install,libvirt-daemon,virt-manager,libvirt,qemu-kvm,libguestfs,guestfs-tools,gcc,gcc-c++,glibc-devel,net-tools,zlib-devel,pciutils,lsof,tcl,tk,git,wget,nano,driverctl,dpdk,dpdk-tools,ipv6calc,wireshark-cli,nmap-ncat,python3-pip,python3-scapy,rpmdevtools,git,netperf,dnsmasq,$locate_pkg" --ks-append="rootpw redhat" --product=$product --retention-tag=$retention_tag --ks-post "$(cat ~/temp/rhts_power.sh)" --arch=aarch64 --machine=$dut --topo=singleHost --systype=machine $(echo "$zstream_repo_list") $(echo "$brew_build_cmd") --param=ROLE="STANDALONE" --param=image_name=$VM_IMAGE_AARCH64 --param=dbg_flag="$dbg_flag" --param=SELINUX=$SELINUX --param=FDP_STREAM=$FDP_STREAM --param=NAY=$NAY --param=PVT=$PVT --param=GET_NIC_WITH_MAC=$GET_NIC_WITH_MAC --param=NIC_MAC_STRING="94:6d:ae:d9:23:f4 94:6d:ae:d9:23:f5" --param=NIC_DRIVER=$NIC_DRIVER --param=RPM_OVS_SELINUX_EXTRA_POLICY=$RPM_OVS_SELINUX_EXTRA_POLICY --param=RPM_OVS=$RPM_OVS --wb "(Standalone: $dut), FDP $FDP_RELEASE, $ovs_rpm_name, $COMPOSE, openvswitch/power_cycle_crash $brew_build $special_info \`ARM Testing, power_cycle_crash Image Mode\`" --append-task="/distribution/utils/power-cycle" --append-task="/kernel/networking/openvswitch/power_cycle_crash/check_config {dbg_flag=set -x} {ROLE=STANDALONE}" --append-task="/kernel/networking/openvswitch/common/misc_tasks/crash_sysrq_c {dbg_flag=set -x}" --append-task="/kernel/networking/openvswitch/power_cycle_crash/check_config {dbg_flag=set -x} {ROLE=STANDALONE}" --append-task=/kernel/networking/openvswitch/common/misc_tasks/recover_pxe_boot
	else
		lstest $KERNEL_TESTS_HOME/kernel/networking/openvswitch/power_cycle_crash/set_config | runtest -B "64k" --task-fetch-url /distribution/check-install@ --fetch-url kernel@https://gitlab.cee.redhat.com/kernel-qe/kernel/-/archive/master/kernel-master.tar.bz2 $COMPOSE --kernel-options "crashkernel=640M" --watchdog-panic "ignore" --ks-append="rootpw redhat" --product=$product --retention-tag=$retention_tag --ks-post "$(cat ~/temp/rhts_power.sh)" --arch=aarch64 --machine=$dut --topo=singleHost --systype=machine $(echo "$zstream_repo_list") $(echo "$brew_build_cmd") --param=ROLE="STANDALONE" --param=image_name=$VM_IMAGE_AARCH64 --param=dbg_flag="$dbg_flag" --param=SELINUX=$SELINUX --param=FDP_STREAM=$FDP_STREAM --param=NAY=$NAY --param=PVT=$PVT --param=GET_NIC_WITH_MAC=$GET_NIC_WITH_MAC --param=NIC_MAC_STRING="94:6d:ae:d9:23:f4 94:6d:ae:d9:23:f5" --param=NIC_DRIVER=$NIC_DRIVER --param=RPM_OVS_SELINUX_EXTRA_POLICY=$RPM_OVS_SELINUX_EXTRA_POLICY --param=RPM_OVS=$RPM_OVS --wb "(Standalone: $dut), FDP $FDP_RELEASE, $ovs_rpm_name, $COMPOSE, openvswitch/power_cycle_crash $brew_build $special_info \`ARM Testing, power_cycle_crash Package Mode\`" --append-task="/distribution/utils/power-cycle" --append-task="/kernel/networking/openvswitch/power_cycle_crash/check_config {dbg_flag=set -x} {ROLE=STANDALONE}" --append-task="/kernel/networking/openvswitch/common/misc_tasks/crash_sysrq_c {dbg_flag=set -x}" --append-task="/kernel/networking/openvswitch/power_cycle_crash/check_config {dbg_flag=set -x} {ROLE=STANDALONE}" --append-task=/kernel/networking/openvswitch/common/misc_tasks/recover_pxe_boot
	fi
fi
