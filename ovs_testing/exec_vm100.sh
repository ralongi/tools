...
if [[ $image_mode == "yes" ]]; then
	lstest $GIT_HOME/kernel/networking/openvswitch/vm100 | runtest --fetch-url kernel@https://gitlab.cee.redhat.com/kernel-qe/kernel/-/archive/master/kernel-master.tar.bz2 $COMPOSE --ks-append="rootpw redhat" --product=$product --retention-tag=$retention_tag --arch=$arch --topo=multiHost.1.1 --machine=$SERVERS,$CLIENTS --systype=$SYSTYPE $(echo "$zstream_repo_list") $(echo "$brew_build_cmd") $(echo "$cmds_to_run") --param=dbg_flag="$dbg_flag" --param=NAY=$NAY --param=PVT=$PVT --param=NIC_DRIVER=mlx5_core --param=RPM_OVS_SELINUX_EXTRA_POLICY=$RPM_OVS_SELINUX_EXTRA_POLICY --param=RPM_OVS=$RPM_OVS --param=VM_IMAGE=$VM_IMAGE --param=NUMBER_OF_GUESTS=$NUMBER_OF_GUESTS --param=GET_NIC_WITH_MAC=$GET_NIC_WITH_MAC --param=mh-NIC_MAC_STRING=$SERVERS_NIC_MAC_STRING,$CLIENTS_NIC_MAC_STRING --bootc=$COMPOSE,frompkg --packages="grubby sshpass iperf3 gcc gcc-c++ glibc-devel net-tools zlib-devel pciutils lsof tcl tk git wget nano driverctl dpdk dpdk-tools ipv6calc wireshark-cli nmap-ncat dnsmasq python3-scapy" --nrestraint --wb "(Server: $SERVERS, Client: $CLIENTS), FDP $FDP_RELEASE, $ovs_rpm_name, $COMPOSE, openvswitch/vm100 $brew_build $special_info \`image mode\`" --insert-task="/kernel/networking/openvswitch/netscout_connect_ports {dbg_flag=set -x} {netscout_pair1=$netscout_pair1} {netscout_pair2=$netscout_pair2}"	
elif [[ $image_mode == "no" ]]; then
	lstest $GIT_HOME/kernel/networking/openvswitch/vm100 | runtest --fetch-url kernel@https://gitlab.cee.redhat.com/kernel-qe/kernel/-/archive/master/kernel-master.tar.bz2 $COMPOSE --ks-append="rootpw redhat" --product=$product --retention-tag=$retention_tag --arch=$arch --topo=multiHost.1.1 --machine=$SERVERS,$CLIENTS --systype=$SYSTYPE $(echo "$zstream_repo_list") $(echo "$brew_build_cmd") $(echo "$cmds_to_run") --param=dbg_flag="$dbg_flag" --param=NAY=$NAY --param=PVT=$PVT --param=NIC_DRIVER=mlx5_core --param=RPM_OVS_SELINUX_EXTRA_POLICY=$RPM_OVS_SELINUX_EXTRA_POLICY --param=RPM_OVS=$RPM_OVS --param=VM_IMAGE=$VM_IMAGE --param=NUMBER_OF_GUESTS=$NUMBER_OF_GUESTS --param=GET_NIC_WITH_MAC=$GET_NIC_WITH_MAC --param=mh-NIC_MAC_STRING=$SERVERS_NIC_MAC_STRING,$CLIENTS_NIC_MAC_STRING --nrestraint --wb "(Server: $SERVERS, Client: $CLIENTS), FDP $FDP_RELEASE, $ovs_rpm_name, $COMPOSE, openvswitch/vm100 $brew_build $special_info" --insert-task="/kernel/networking/openvswitch/netscout_connect_ports {dbg_flag=set -x} {netscout_pair1=$netscout_pair1} {netscout_pair2=$netscout_pair2}"	
fi
