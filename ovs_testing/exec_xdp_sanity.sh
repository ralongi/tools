...
if [[ $netscout_pair1 ]] || [[ $netscout_pair2 ]]; then
	...
	# Brew command:
	lstest ~/git/kernel/networking/ebpf_xdp/sanity | runtest $COMPOSE --crashsize=640M --product=$product --retention-tag=$retention_tag --arch=x86_64 --topo=multiHost.1.1 --machine=$server,$client --systype=$SYSTYPE,$SYSTYPE $(echo "$zstream_repo_list") --cmd="grubby --args=crashkernel=640M --update-kernel=ALL"  -B 'repo:myrepo,https://artifacts.internal.cki-project.org/arr-cki-prod-internal-artifacts/internal-artifacts/1362974800/publish_x86_64/7279296924/artifacts/repo/5.14.0-284.74.1.rt14.359.1855_1362974788.el9_2.x86_64/ rtk'  --param=DBG_FLAG="$DBG_FLAG" --param=SKIP_CASE="$SKIP_CASE" --param=NAY=yes --param=NIC_NUM=2 --param=mh-NIC_DRIVER="${server_driver}","${client_driver}" $(echo "$pciid_info") --param=mh-TEST_DRIVER="${server_driver}","${client_driver}" --param=XDP_LOAD_MODE="$XDP_LOAD_MODE" --param=XDP_TEST_FRAMEWORK="$XDP_TEST_FRAMEWORK" --wb "(Server/DUT: $server, Client: $client), XDP Sanity test, $COMPOSE, networking/ebpf_xdp/sanity, Client driver: $client_driver, Server driver: $server_driver, Driver under test: $server_driver ($card_info) $special_info" --append-task="/kernel/networking/openvswitch/crash_check {dbg_flag=set -x}"
