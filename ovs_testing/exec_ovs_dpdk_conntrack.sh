...
dbg_flag="set -x"
testdir="/home/ralongi/git/kernel/networking/ovs-dpdk/ovs-dpdk-conntrack"
pushd $testdir
...
#lstest | runtest $compose  --variant=server --arch=x86_64 --machine=$server --systype=machine  --param=dbg_flag="set -x" --param=RPM_CONTAINER_SELINUX_POLICY=$RPM_CONTAINER_SELINUX_POLICY --param=QCOW_LOC=China --param=DPDK_URL=$RPM_DPDK --param=DPDK_TOOL_URL=$RPM_DPDK_TOOLS --param=QEMU_KVM_RHEV=$QEMU_KVM_RHEV --param=xena_port1=XENA_M7P0 --param=xena_port2=XENA_M7P1 --param=GUEST_DPDK_VERSION=18-11 --param=NetScout_nic1=dell51_p4p1 --param=NetScout_nic2=dell51_p4p2 --param=NIC1_NAME=p4p1 --param=NIC2_NAME=p4p2 --param=xena_module=7 --param=GUEST_IMG=$image_name --param=OVS_URL=$RPM_OVS --param=OVS_EXTRA=$RPM_OVS_SELINUX_EXTRA_POLICY --wb "$fdp_release, $ovs_rpm_name, $compose, $(echo $testdir | awk -F 'git' '{print $2}')"
