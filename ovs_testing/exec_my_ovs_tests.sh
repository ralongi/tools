...
if [[ -z $RPM_OVS_TCPDUMP_PYTHON ]]; then
	export RPM_OVS_TCPDUMP_PYTHON=$OVS360_PYTHON_25D_RHEL10
fi
if [[ -z $RPM_OVS_TCPDUMP_TEST ]]; then
	export RPM_OVS_TCPDUMP_TEST=$OVS360_TCPDUMP_25D_RHEL10
fi
...
if [[ -z $VM_IMAGE_AARCH64 ]]; then
	export VM_IMAGE_AARCH64="rhel10.1.aarch64.qcow2"
else
...
# OVS packages
if [[ -z $RPM_OVS ]]; then
	export RPM_OVS=$OVS360_25D_RHEL10
else
...
# OVN packages
export RPM_OVN=$OVN360_25D_RHEL10 
...
sedeasy "25D" "$FDP_RELEASE" ~/github/tools/ovs_testing/exec_perf_ci.sh
sedeasy "25D" "$FDP_RELEASE" ~/github/tools/ovs_testing/exec_endurance.sh
