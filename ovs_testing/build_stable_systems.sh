...
if [[ $RHEL_VERSION -eq 9 ]]; then
	...
	bkr workflow-tomorrow 9 --split-buildroot --hostrequire=hostlabcontroller=lab-02.rhts.eng.rdu.redhat.com --stable-system --taskparam OATS_TPS_STABLE=true --profile=fast-datapath-for-rhel-9-rpms --arch=x86_64 --variant=BaseOS --task "! dnf -y install openvswitch-selinux-extra-policy tcpdump libatomic" --task "! /bin/cp -f /etc/tpsd.conf /etc/tpsd.conf_orig" --task "! wget -O /etc/tpsd.conf http://netqe-infra01.knqe.eng.rdu2.dc.redhat.com/misc/tpsd.conf" --task "! systemctl restart tpsd" --task "! dnf -y install wget; wget -O ~/tps_rhel9_packages.sh http://netqe-infra01.knqe.eng.rdu2.dc.redhat.com/share/ralongi/tps_rhel9_packages.sh; chmod +x ~/tps_rhel9_packages.sh; ~/tps_rhel9_packages.sh" --task "! wget -O ~/stable_cleanup.sh http://netqe-infra01.knqe.eng.rdu2.dc.redhat.com/share/ralongi/stable_cleanup.sh; chmod +x ~/stable_cleanup.sh"
