...
if [[ $FDP_RELEASE_ALPHA == "A" ]]; then
	...
	if [[ ! $(grep -i "$STARTING_FDP_RELEASE" ~/git/my_fork/kernel/networking/openvswitch/common/package_list.sh) ]]; then
...
if [[ -z $OVS_LATEST_STREAM_PKG ]]; then
	...
	export OVS_LATEST_STREAM_PKG=$(tail -n +$start_line ~/git/my_fork/kernel/networking/openvswitch/common/package_list.sh | grep OVS | grep RHEL"$COMPOSE_VER" | tail -3 | head -n1 | awk -F "=" '{print$2}')
fi
