...
cat /home/ralongi/github/tools/get_compose_id/get_compose_id_special.sh | sshpass -p $password ssh -q -o "StrictHostKeyChecking= no" root@$target 'bash -'

sshpass -p $password ssh -q -o "StrictHostKeyChecking= no" root@$target cat ~/gci_output.txt
#export rhel_compose=$(sshpass -p $password ssh -q -o "StrictHostKeyChecking= no" root@$target cat gci_output.txt)
